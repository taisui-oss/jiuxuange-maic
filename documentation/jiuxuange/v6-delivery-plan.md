...
- 1000 人、花名企微、无强制反思、3-6 题两次、麦客思开发卡已记录；
- 已实现、已验证、仅设计、尚未完成分开；
...
真实学员登录
→ 进入商业模式大课
→ 打开第一个 OpenMAIC 原生案例课堂
→ 完成 required 原生互动和客观题
→ 服务端记录当前案例完成
→ 解锁下一个案例
→ 进入下一个独立案例课堂
→ 换设备恢复
...
3. 建立 Course、CourseVersion、CaseProgress、InteractionAttempt、CaseUnlock；
4. 停用正式入口中的自定义 V5.1 补偿状态机；
5. 保留旧课堂只读和回退；
6. 为每个案例建立独立的多轮原生课堂包与稳定路由；
7. 建立“当前案例完成后解锁下一案例”的顺序配置；
8. 建立跨设备恢复；
9. 完成桌面、移动端和断网重试 E2E。

Gate 2 硬条件：

- 当前案例 required 客观题未答对时不得完成；
- 不建立第二份互相冲突的进度状态；
- 重复提交和双标签页不得重复完成或解锁；
- 不得越过未完成的前序案例；
- 未解锁案例不能通过直接 URL 进入；
...
1. 学员端项目卡草稿、预览、案主确认和发布；
2. 后台批量导入草稿与导入来源审计；
3. 事实、判断、假设、未知项和材料分层；
4. 对象存储和材料权限；
5. `allow/mask/block` 上下文生成器；
6. 个人研习私有会话；
7. 小组共享讨论线程；
8. 个人笔记草稿与保存；
9. 项目记录草稿、确认和版本；
...
1. 将现有 L0 商业模式六题、两次提交领域合同迁移为首个 PostgreSQL 配置；
2. 建立 3-6 题数量约束、五类题型、题目集和量表版本；
3. 建立草稿自动保存；
4. 禁用测试页实时 Agent；
5. 提交事务与评价 Job 分离；
...
- 每个发布题目集包含 3-6 题，商业模式首发为 6 题；
- 只允许两次正式提交；
- 草稿不消耗机会；
- AI 失败不改变提交状态；
- 重试只生成一份评价；
- 测评、测评会话和项目卡必须使用相同的 `project_id`；
- 测评会话必须冻结项目卡的 `project_card_version_id`；
- 评价引用冻结题目、答案、项目卡和量表版本；
- 不显示数字总分；
- 不读取个人研习或小组讨论历史；
- 不输出人格、潜力、晋级或淘汰结论。

Gate 3 与 Gate 4 的完整产品合同：

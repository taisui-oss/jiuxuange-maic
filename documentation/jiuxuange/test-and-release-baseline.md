...
| 项目讨论 | 未实现 | 个人私有 + 小组共享 |
| 双档案 | 未实现 | 个人笔记与项目档案独立 |
| 个人测试 | 商业模式固定六题、两次 L0 | 每门课 3-6 题配置 + PostgreSQL + 异步 AI 评价 |
...
- 七模块项目卡必填、案主确认和后台导入待确认测试；
- 项目卡 V1/V2 与历史会话、测评的版本隔离测试；
- `allow / mask / block` canary 零命中测试；
- 3 题、6 题合法题目集发布测试；
- 少于 3 题、多于 6 题拒绝测试；
- 五类题型元数据与课程目标引用测试；
- 商业模式首发 6 题测试；
- 测评页无 Agent、无讨论历史输入测试；
- 评价引用答案片段与项目事实测试；
- AI 失败、重试、Worker 重启和单份评价幂等测试。
...
### 案例与项目卡配置预览基线（2026-07-28）

`eval_run_id`: `jgx-v6-case-project-config-preview-20260728-01`

| 验证项 | 结果 |
|---|---|
| 八步分析路径 | 案例目录、社区早餐连锁课堂、麦客思项目卡和个人测评口径一致 |
| 首个原生课堂 | `jxg-bm-case-breakfast-chain-six-elements-v1`，10 个场景，5 道原生客观互动 |
| 课程包 SHA-256 | `0994ff23584c6cc501a717ade7fc70e36bae7168a32d2ad203ac8dca773d7a4f` |
| 顺序解锁 | 社区早餐连锁初始开放；完成后才开放便利蜂；审校中案例保持不可进入 |
| 项目卡 | `project-card-mckess@1.1.0-draft`，七模块，字段级 `allow / mask / block` |
| 个人测评 | `bm-assessment-mckess-v2`，六题、五类题型、最多两次提交、无实时 Agent |
| 专项自动化 | 6 files / 32 tests passed |
| 受控全量自动化 | `--maxWorkers=4`，307 files / 2289 tests passed |
| TypeScript | `pnpm exec tsc --noEmit` passed |
| 变更范围 ESLint | 0 error、0 warning |
| Production build | 47 个页面/路由，standalone 资源已准备 |
| 本机浏览器 | 首页入口、八步目录、首个课堂、七模块项目卡和六题测评通过 |
| 移动端 | 390 × 844 课程目录与测评页无横向溢出 |
| 公开预览脱敏模拟 | `block` 字段、完整事实、原始页码和精确财务值均未出现在响应中 |

[KNOWN, HIGH] 该基线只证明当前公开预览配置和本机 L0 界面成立。麦客思数据仍是
